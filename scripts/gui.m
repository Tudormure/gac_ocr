    ...
    function predict_callback(~, ~)
        ...
        if poz*100 < 80
            set(lbl_rezultat, 'String', ...
              sprintf('Predictie: %d (Siguranta: %.1f%%) (nu prea sigur)', val-1, poz*100));
        else
            set(lbl_rezultat, 'String', ...
              sprintf('Predictie: %d (Siguranta: %.1f%%) (destul de sigur)', val-1, poz*100));
        end
